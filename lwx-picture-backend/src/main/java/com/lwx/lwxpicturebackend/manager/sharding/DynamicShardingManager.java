package com.lwx.lwxpicturebackend.manager.sharding;
...
public class DynamicShardingManager {
    ...
    @PostConstruct
    public void initialize() {//用于在项目初始化过程中获取所有的分表以及原表名
        log.info("初始化动态分表配置...");
    ...
    /**
     * 更新 ShardingSphere 的 actual-data-nodes 动态表名配置PictureShardingAlgorithm
     */
    private void updateShardingTableNodes() {
        Set<String> tableNames = fetchAllPictureTableNames();
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "lwx_picture." + tableName) // 确保前缀合法
