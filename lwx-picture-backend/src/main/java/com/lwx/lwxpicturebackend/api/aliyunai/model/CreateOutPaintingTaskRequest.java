...
//@JsonProperty("xScale")注解的作用：
// 举个例子，根据JavaBean 的规范，属性名称与其访问器方法(getter 和 setter)之间的映射规则是:
// 如果属性名以小写字母开头，第二个字母是大写(如 eai1 )，规范仍认为属性名称是 emai1，
// 而访问器方法应为 getemai1()和 setemai1()。
// 但Jackson 会尝试推断属性名为 emai1(因为 eMai1不常见)，从而导致JSON中 emai1或 emai1 可能无法正确映射。
